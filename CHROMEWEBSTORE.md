# Chrome Web Store Listing & Privacy Disclosures
...
Collect copied text snippets, manage personal contact info (Name, Email, Phone, Address), and auto-fill web forms with 1-click.

### Detailed Store Description
ClipFill is a smart clipboard companion and form filling assistant designed to supercharge your web productivity. 

#### Key Features:
- 📋 **Automatic Clipboard Collection**: Automatically saves copied text snippets into a clean, searchable sidebar history.
...
| --- | --- |
| `sidePanel` | Required to render the ClipFill sidebar alongside active web pages for quick snippet selection and form filling. |
| `storage` | Required to save clipboard history, user snippets, category tags, and user preferences locally on the browser. |
| `contextMenus` | Required to allow users to right-click selected text on web pages and save it directly to ClipFill. |
| `tabs` | Required to query the active tab context and inspect web page forms for accurate field auto-filling. |
...
| `<all_urls>` | Required to allow form filling and text selection monitoring across all web pages visited by the user. |
...
- **v1.0.0** (2026-08-16): Initial production release featuring side panel UI, copy auto-capture, targeted field picker, smart form inspector, context menu integration, and local storage management.
